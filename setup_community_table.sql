-- 连接到数据库并创建社区减灾能力表
-- 请复制以下SQL语句到MySQL客户端或数据库管理工具中执行

-- 1. 连接到数据库
-- mysql -h 192.168.15.203 -P 30314 -u root -p

-- 2. 选择数据库
USE evaluate_db;

-- 3. 创建社区行政村减灾能力表
CREATE TABLE IF NOT EXISTS `community_disaster_reduction_capacity` (
...
-- 6. 插入一条测试数据（可选）
INSERT INTO `community_disaster_reduction_capacity` (
  `region_code`, `province_name`, `city_name`, `county_name`, `township_name`, `community_name`,
  `has_emergency_plan`, `has_vulnerable_groups_list`, `has_disaster_points_list`, `has_disaster_map`,
  `resident_population`, `last_year_funding_amount`, `materials_equipment_value`,
  `medical_service_count`, `militia_reserve_count`, `registered_volunteer_count`,
  `last_year_training_participants`, `last_year_drill_participants`, `emergency_shelter_capacity`
) VALUES (
  '510122001', '四川省', '成都市', '双流区', '西航港街道', '测试社区',
  '是', '是', '是', '否',
  1000, 50.50, 30.20,
  2, 50, 100,
  200, 150, 500
...
-- 7. 查询测试数据
SELECT * FROM `community_disaster_reduction_capacity` WHERE `region_code` = '510122001';