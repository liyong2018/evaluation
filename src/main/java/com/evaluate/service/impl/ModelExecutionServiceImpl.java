...
public class ModelExecutionServiceImpl implements ModelExecutionService {
    ...
    public List<Map<String, Object>> generateResultTable(Map<String, Object> executionResults) {
        ...
        for (String regionCode : allRegions) {
            ...
            // 琛岀骇鍒細绀惧尯 or 涔￠晣锛屼緵鍓嶇鎸夋楠よ繃婊よ
            if (regionCode.startsWith("TOWNSHIP_")) {
    ...
    // 鏂扮増锛氭壂鎻忔墍鏈夎锛屽悎骞跺垪锛屽啀鏍规嵁 stepOutputParams 鍙嶆爣璁?stepOrder锛岄伩鍏嶉琛屼笉鍖呭惈鍏ㄩ儴姝ラ鍒楀鑷寸己澶?    private List<Map<String, Object>> generateColumnsWithAllStepsV2(
            List<Map<String, Object>> tableData,
            Map<Integer, List<String>> stepOutputParams) {
        List<Map<String, Object>> columns = new ArrayList<>();
        if (tableData == null || tableData.isEmpty()) {
        ...
        // 鍩虹鍒?        Set<String> baseColumns = new LinkedHashSet<>(Arrays.asList("regionCode", "regionName", "region"));
        ...
        for (Map.Entry<String, List<Map<String, Object>>> entry : townshipGroups.entrySet()) {
            ...
            for (StepAlgorithm algorithm : algorithms) {
                ...
                for (Map<String, Object> community : communities) {
                    ...
                    // 褰撹〃杈惧紡瀛楁涓嶅瓨鍦ㄦ椂锛屽洖閫€浣跨敤璇ョ畻娉曠殑杈撳嚭鍙傛暟鍚?                    if (value == null && outputParam != null && !outputParam.isEmpty()) {
    ...
    // Derive 9 township inputs from community fields when missing
    private Map<String, Double> deriveCommunityIndicators(Map<String, Object> c) {
        Map<String, Double> r = new HashMap<>();
        Double popVal = number(c, "RESIDENT_POPULATION");
        if (popVal == null) popVal = number(c, "resident_population");
        double pop = (popVal != null && popVal > 0.0) ? popVal : 1.0;
    ...
    private Double number(Map<String, Object> c, String k) {
        ...
        if (v instanceof String) { try { return Double.parseDouble((String) v); } catch (Exception ignore) {} }
        return null;
    }
