...
# ============================================================
# 减灾能力评估系统 - 部署脚本集合
# Disaster Reduction Evaluation System - Deployment Scripts
# ============================================================
#
# 使用方法:
#   ./scripts/deploy.sh            # 完整部署（生产环境）
#   ./scripts/deploy.sh remote     # 显示远程部署说明
#
# 环境变量:
...
#   SSH_KEY        - SSH密钥路径 (可选)
#   COMPOSE_FILE   - Docker Compose文件 (默认: docker-compose.prod.yml)
#
# ============================================================
...
show_help() {
    ...
    echo "命令:"
    echo "  (无参数)    完整部署（生产环境）"
    echo "  remote      显示远程部署说明"
    echo "  help        显示此帮助信息"
    echo ""
    echo "环境变量:"
    echo "  SERVER_HOST    服务器地址 (默认: 101.126.46.254)"
    echo "  SERVER_USER    服务器用户 (默认: root)"
    echo "  REMOTE_DIR     远程目录 (默认: /opt/evaluation)"
    echo "  SSH_KEY        SSH密钥路径 (可选)"
    echo "  COMPOSE_FILE   Docker Compose文件 (默认: docker-compose.prod.yml)"
    echo ""
    echo "示例:"
    echo "  SERVER_HOST=192.168.1.100 SERVER_USER=admin ./scripts/deploy.sh"
}
...
show_remote_instructions() {
    echo -e "${GREEN}=========================================${NC}"
    echo -e "${GREEN}   Remote Deployment Instructions${NC}"
    echo -e "${GREEN}=========================================${NC}"
    echo ""
    echo -e "${YELLOW}Step 1: Connect to remote server${NC}"
    echo "  ssh $SERVER"
    echo ""
    echo -e "${YELLOW}Step 2: On remote server, run:${NC}"
    echo "  cd $REMOTE_DIR"
    echo "  git pull"
