...
| overwrite | boolean       | false | 是否覆盖已有数据    |
...
| importCount | integer | 导入记录数  |
| errorList   | array   | 错误信息列表 |

```
POST /api/data/import/weight
```
...
| 参数名        | 参数类型          | 是否必需 | 描述          |
| ---------- | ------------- | ---- | ----------- |
| file       | MultipartFile | true | 权重配置Excel文件 |
...
| -------- | ------- | ------ |
| success  | boolean | 导入是否成功 |
| configId | long    | 权重配置ID |

**数据查询相关**

```
GET /api/data/survey/list
```
...
```
GET /api/data/weight/indicator/tree/{configId}
```
...
```
POST /api/data/weight/indicator/update
```
...
| 参数名        | 参数类型  | 是否必需 | 描述     |
| ---------- | ----- | ---- | ------ |
| configId   | long  | true | 权重配置ID |
| indicators | array | true | 指标权重列表 |
...
| calculateAll   | boolean | false | 是否计算所有数据 |
...
| resultCount | integer | 计算结果数量 |
| message     | string  | 计算结果信息 |

```
GET /api/evaluation/secondary/list
```
...
```
GET /api/evaluation/primary/list
```
